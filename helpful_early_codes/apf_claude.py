...
def main():
    ...
    mag_diff = 10   # Difference between path magnitudes
    ...
    while True:
        ...
        # Update magnitude based on which path is better
        if current_best_length < best_path_length:
            ...
            if path_lengths[0] < path_lengths[1]:
                magnitude = max(1, magnitude - mag_diff)
            else:
                magnitude += mag_diff
            prev_magnitude = magnitude    
            print(f"New best path length: {best_path_length}, magnitude: {magnitude}")
        else:
            ...
            if consecutive_no_improvement >= max_no_improvement:
                ...
                consecutive_no_improvement = 0
                print(f"No improvement for {max_no_improvement} iterations, reverting to magnitude: {magnitude}")
